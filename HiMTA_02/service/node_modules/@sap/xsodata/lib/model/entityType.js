...
/**
 * Adds a concurrent property to the list of concurrent properties.
 *
 * @param prop {string} The name of the property to add
 */
EntityType.prototype.addConcurrentProperty = function addConcurrentProperty(prop) {
    this.concurrentProperties[prop] = true;
};
...
EntityType.prototype.setKeyNames = function (keyNames) {
    ...
    // Validates that current keys are not part of xsodata concurrencytoken configuration
    if (this.hasConcurrentProperties()) {
        updateConcurrentPropertyOnSetKeyNames(this);
...
EntityType.prototype.init = function (metadata, kind, tableStoreType, logger) {
    ...
    if (this.hasConcurrencyToken()) {
        addMissingConcurrentProperties(this);
...
EntityType.prototype.setCalculationView = function (rows, calcView, logger) {
...
function updateConcurrentPropertyOnSetKeyNames(entityType) {

    var origTokens, intersection;

    if (!entityType.hasConcurrencyToken()) {
        return;
    }

    origTokens = Object.keys(entityType.originalConcurrentProperties);

    intersection = lodash.intersection(entityType.keys.names, origTokens);
    ...
    entityType.concurrentProperties = lodash.omit(entityType.concurrentProperties,
        entityType.keys.names);
}
 ...
 *
 * @private
 * @param entityType {object} The entity to update
 */
function addMissingConcurrentProperties(entityType) {

    if (!entityType.hasConcurrentProperties()) {
        entityType.propertyNames.forEach(function (propertyName) {
            entityType.addConcurrentProperty(propertyName);
        });
    }

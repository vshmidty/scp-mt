...
jsonToDb['Edm.Time'] = function jsonToDbEdmTime(jsonValue, dbDataType) {
    /* in     "PT0H0M0S"                        */
    /* out    "00:00:00"                        */
    /* in     "PT23H59M59S"                     */
    /* out    "23:59:59"                        */
    ...
    var match = /PT([0-9]+)H([0-9]+)M([0-9]+)S/i.exec(jsonValue);

    return (match[1].length === 2 ? match[1] : '0' + match[1]) + ':' +
        (match[2].length === 2 ? match[2] : '0' + match[2]) + ':' +
        (match[3].length === 2 ? match[3] : '0' + match[3]);
};
...
jsonToDb['Edm.String'] = function jsonToDbEdmString(jsonValue, dbDataType) {
    ...
    if (dbDataType === 'VARCHAR') {
        ...
        /* in   "CLOB"                          */
        /* out buffer                           */
        return new Buffer(jsonValue, 'utf8');
...
jsonToDb['Edm.Binary'] = function jsonToDbEdmBinary(jsonValue, dbDataType) {
    ...
    if (dbDataType === 'BLOB') {
        /* in  "iVBORg=="                       */
        /* out buffer                           */
        return new Buffer(jsonValue, 'base64');

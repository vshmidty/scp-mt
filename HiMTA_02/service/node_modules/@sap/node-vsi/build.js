...
var modPath = platform+ '-'+ arch+ '-'+ v8;
if( arch === 'ppc64' ) {
	var os = require('os');
	var endianer = os.endianness().toLowerCase();
	if( endianer === 'le' ) {
		modPath = platform+ '-'+ arch + endianer + '-'+ v8;
	}
}
if (!force) {
	try {
		fs.statSync(path.join(__dirname, 'bin', modPath, 'vsi.node'));
...
function afterBuild() {
	var targetPath = path.join(__dirname, 'build', debug ? 'Debug' : 'Release', 'vsi.node');
	var installPath = path.join(__dirname, 'bin', modPath, 'vsi.node');

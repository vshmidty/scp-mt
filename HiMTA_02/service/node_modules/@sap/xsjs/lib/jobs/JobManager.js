...
JobManager.prototype.startJobAsync = function (rt, job, request) {
  ...
  if (job.action.isJavaScript()) {
    runnable = this._createXsjsRunnable(rt, job, request, jobRunDetails.runId);
  } else {
...
JobManager.prototype._createXsjsRunnable = function (rt, job, req, runId) {
  var runner = new XsjsJobRunner(rt);
  runner.prepare(job, req, runId);
